...
def _safe_next(value: str | None) -> str:
    return value if value and value.startswith("/admin/") and not value.startswith("//") else "/admin"
...
def ready() -> JSONResponse:
    checks: dict[str, bool] = {"postgres": False, "qdrant": False, "storage": False, "models": False}
    try:
    ...
    checks["models"] = not settings.model_local_files_only or any(settings.model_cache_dir.iterdir())
